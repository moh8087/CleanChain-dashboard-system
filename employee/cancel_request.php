...
// Display teacher menu:
	...
	if ( (isset($_GET['id'])) && (is_numeric($_GET['id'])) ) { // From teacher.php
		...
		$r = mysqli_query ($dbc, $q) or trigger_error("Query: $q\n<br />MySQL Error: " . mysqli_error($dbc));
			...
			echo '<form action="cancel_request.php" method="post">';

			echo '<div class="panel panel-danger">
      <div class="panel-heading"> Are you sure to cancel this order?</div>
	...
	<br>
    <button type="submit" class="btn btn-danger btn-lg" name="submit">Cancel Order</button>
...
';
	...
	if ($trimmed['active'] == 1) { // If the choice is OK for delete test...
			...
			$q = "UPDATE requests SET status='$received' WHERE id='$id'";
        
			$r = mysqli_query ($dbc, $q) or trigger_error("Query: $q\n<br />MySQL Error: " . mysqli_error($dbc));

			if (mysqli_affected_rows($dbc) == 1) { // If it ran OK.
			  
			     echo '<div class="alert alert-warning">Order successfully Cancelled</div>';
				include ('includes/footer.html'); // Include the HTML footer.
				exit(); // Stop the page.
				
				
			} else { // If it did not run OK.
				echo '<div class="alert alert-danger">Cannot recicve this order </div>';
			}
