...
if (empty($_REQUEST["fullname"]) || empty($_REQUEST["email"]) || empty($_REQUEST["mobile"]) || empty($_REQUEST["password"])) {
...
$sure = $access->getUserByMobile($mobile);

if(!$sure["id"]) { // if there is no user

  // STEP 4. Insert user information
  $result = $access->registerUser($email, $secured_password, $salt, $mobile, $fullname);

  // successfully registered
  if ($result) {

      // get current registered user information and store in $user
      $user = $access->selectUserByMobile($mobile);
  ...
  $returnArray["status"] = "400";
  $returnArray["message"] = "Mobile is already taken";
