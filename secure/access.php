...
class access {
    ...
    public function selectUser($email) {
     ...
     // Select user information by mobile
    public function selectUserByMobile($mobile) {
    ...
    public function getUser($email) {
     ...
     // Get full user information by mobile
    ...
    // Updating password for Driver
    public function updatePasswordDriver($did, $password, $salt) {

        $sql = "UPDATE drivers SET password=?, salt=? WHERE did=?";
        $statement = $this->conn->prepare($sql);
        ...
        $statement->bind_param("ssi", $password, $salt, $did);
    ...
    public function getClosestDriver( $rLat, $rLong) {
        ...
        // Trim dlat and dlog
              // trim latitude and longitude
                $row["dlong"]= round($row["dlong"],6);
                $row["dlat"]= round($row["dlat"],6);
    ...
    public function getRequestInfo($id) {
        ...
        // sql statement
        $sql = "SELECT id, email, rid, rname, rmobile, did, demail, dname, dmobile, dphoto, carmodel, dlat , dlog, droplatitude, droplongitude FROM requests WHERE id='".$id."'";
    ...
    public function cancelRequest($status, $id) {
     ...
     // Rate Request according to request id 
    public function rateRequest($rate, $id) {

      // sql statement
      $sql = "UPDATE requests SET rate=? WHERE id=?";
      ...
      // bind parameters to sql statement
      $statement->bind_param("si", $rate, $id);
    ...
    // Add Price with Total Charge
    public function savePrice($price,$status, $id) {
        
        
        $company_portion = $price * 0.20;
        $driver_portion =  $price - $company_portion;

      // sql statement
      $sql = "UPDATE requests SET totalcharge=?, driver_portion=?, company_portion=? , status=? WHERE id=?";
      ...
      // bind parameters to sql statement
      $statement->bind_param("ssssi", $price, $driver_portion, $company_portion, $status, $id);
    ...
    // to calculate closest driver
    public function savePriceByKilo( $rLat, $rLog, $dLat, $dLog ) {
        ...
        $dLog1 = (double)$dLog1;
          ...
          $theta = $rLog1 - $dLog1;
          $dist = sin(deg2rad($rLat1)) * sin(deg2rad($dLat1)) +  cos(deg2rad($rLat1)) * cos(deg2rad($dLat1)) * cos(deg2rad($theta));
          $dist = acos($dist);
          $dist = rad2deg($dist);
          $miles = $dist * 60 * 1.1515;

          // convert to from Meters to Kilometer
          $miles = $miles / 1000;

          // if we assume one Kilometer is 1.5 Riyal
          // if you want the first Kilometer by 5 riyals
          $miles = ($miles * 1.5) + 5;
    ...
    public function tripInfoDriver($id) {
        ...
        // sql statement
        $sql = "SELECT id, email, rid, rname, rmobile, did, dname, dmobile, dphoto, carmodel, picklatitude, picklongitude,  dlat , dlog, totalcharge, status FROM requests WHERE id='".$id."'";
    ...
    public function getMyTrips($rid) {
        ...
        // sql statement
        $sql = "SELECT id, dname, dmobile, pickaddress, notes, dlat , dlog ,totalcharge, status, created_at FROM requests WHERE rid='".$rid."' ORDER BY id DESC" ;
