...
class access {
    ...
    public function savePrice($price,$status, $id) {
      ...
      // sql statement
      $sql = "UPDATE requests SET totalcharge=? , status=? WHERE id=?";
      ...
      // bind parameters to sql statement
      $statement->bind_param("ssi", $price, $status, $id);
    ...
    public function savePriceByKilo( $rLat, $rLog, $dLat, $dLog ) {
          ...
          $theta = $rLog - $dLog;
          $dist = sin(deg2rad($rLat)) * sin(deg2rad($dLat)) +  cos(deg2rad($rLat)) * cos(deg2rad($dLat)) * cos(deg2rad($theta));
          $dist = acos($dist);
          $dist = rad2deg($dist);
          $miles = $dist * 60 * 1.1515;
