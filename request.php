...
if (empty($_REQUEST["rid"]) || empty($_REQUEST["email"])|| empty($_REQUEST["rname"]) || empty($_REQUEST["rmobile"]) || empty($_REQUEST["pickaddress"]) || empty($_REQUEST["picklatitude"]) || empty($_REQUEST["picklongitude"]) || empty($_REQUEST["notes"])) {
...
$dropaddress = "No drop dist";
$notes = htmlentities($_REQUEST["notes"]);


// trim latitude and longitude
$picklatitude= round($picklatitude,6);
$picklongitude= round($picklongitude,6);
...
if ($requestIsAdded)
{
  ...
  $details = array();
  $details["subject"] = "New Order";
  $details["to"] = "washingchain@gmail.com";
...
$details["body"] .= "<tr><td><strong>Customer name: </strong> </td><td>" . $request["rname"] . "</td></tr>";
$details["body"] .= "<tr><td><strong>Customer mobile:</strong> </td><td>" . $request["rmobile"] . "</td></tr>";
