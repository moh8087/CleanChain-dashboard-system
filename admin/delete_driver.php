...
// Display teacher menu:
	...
	if ( (isset($_GET['id'])) && (is_numeric($_GET['id'])) ) { // From teacher.php
		...
		$q = "SELECT did FROM drivers WHERE (did='$id')";		
		$r = mysqli_query ($dbc, $q) or trigger_error("Query: $q\n<br />MySQL Error: " . mysqli_error($dbc));
			...
			echo '<form action="delete_driver.php" method="post">';

			echo '<div class="panel panel-danger">
      <div class="panel-heading"> Are you sure to delete this delegate?</div>
...
';
	...
	if ($trimmed['active'] == 1) { // If the choice is OK for delete test...
			...
			$q = "DELETE from drivers  WHERE did='$id'";
			$r = mysqli_query ($dbc, $q) or trigger_error("Query: $q\n<br />MySQL Error: " . mysqli_error($dbc));

			if (mysqli_affected_rows($dbc) == 1) { // If it ran OK.
			  
			     echo '<div class="alert alert-success">Delegate successfully deleted</div>';
				include ('includes/footer.html'); // Include the HTML footer.
				exit(); // Stop the page.
				
				
			} else { // If it did not run OK.
				echo '<div class="alert alert-danger">Cannot delete this delegate </div>';
			}
